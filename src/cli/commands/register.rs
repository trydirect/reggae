...
use crate::cli::commands::output::{print_domain, OutputFormat};
use crate::core::{registry::ProviderRegistry, error::Error, types::{DnsRecord, DnsRecordType}};

pub async fn run(args: &RegisterArgs, global: &GlobalArgs, registry: &ProviderRegistry) -> Result<(), Error> {
    ...
    let domain = provider.register_domain(&args.domain, initial).await?;
